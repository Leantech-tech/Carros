...
class CarrosSupabaseUser extends BaseAuthUser {
  CarrosSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> carrosSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CarrosSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
